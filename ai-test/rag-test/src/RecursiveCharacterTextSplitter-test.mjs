...
/**
 * RecursiveCharacterTextSplitter 的目标
 * 尽量 保留语义完整性，优先按“大块→中块→小块”的层级递归切分，直到满足 chunkSize。
 */
const logTextSplitter = new RecursiveCharacterTextSplitter({
    chunkSize: 150,
    chunkOverlap: 20,
    separators: ['\n', '。', '，'],
    lengthFunction: (text) => enc.encode(text).length // 严格控制 token 大小，比如需要计费的场景，就可以实现 lengthFunction 用 token 的方式计算长度
});
