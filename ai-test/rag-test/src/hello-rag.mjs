...
/**
 * Document 是 LangChain 中的标准文档数据结构;
 * 在 RAG（Retrieval-Augmented Generation）架构里：
 * 文档 → 切块 → 向量化 → 存储 → 检索 → 提示拼接 → LLM
 * new Document({
    pageContent: string,
...
/**
 * API: fromDocuments 初始化并构建
 * 基于 embeddings 模型把文档向量化存入数据库
 */
...
const retrievel = vectorStore.asRetriever({ k: 3 });

const questions = ["东东和光光是怎么成为朋友的？"];
// const questions = ["光光和东东最后怎么样了？"];
...
for (const question of questions) {
  ...
  // 使用 retriever 获取文档
  const retrievedDocs = await retrievel.invoke(question);
  /**
   * API: similaritySearch(query, k)相似度搜索
