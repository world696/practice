 ...
 * 如果当前对话上下文超过阈值，就自动对历史对话进行摘要压缩，剔除冗余信息
 * 只保留关键上下文摘要，在传入上下文进行独写/问答；
 */
...
const agent = createAgent({
  ...
  middleware: [
    createSummarizationMiddleware({
       ...
       * 触发摘要的方式
       * messages 按消息条数；整数条数
       * tokens 按token数；整数token数；
       * fraction 按默认上下文创建的比例；0-1的小数
       */
      // 低阈值便于 demo 触发摘要；生产环境可省略 trigger/keep，由模型 profile 自动推断
      trigger: { type: "messages", value: 8 }, // 根据消息条数进行统计，8条就触发摘要
      keep: { type: "messages", value: 4 }, // 保留四条，前面的变更成摘要
    }),
  ],
});
