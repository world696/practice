...
const HybridRetrievalState = Annotation.Root({
  query: Annotation(), // 用户原问题
  queryAugmentation: Annotation(), // 改写的问题
  esHits: Annotation(), // es去重的结果
  milvusHits: Annotation(), // Milvus去重的结果
  merged: Annotation(), // 合并去重的结果
  topDocuments: Annotation(), // 重排的topN
  answer: Annotation(), // 最终回答字符串
});
...
export function compileHybridRetrievalGraph(esClient, milvus, reranker, chatModel) {
  ...
  return new StateGraph(HybridRetrievalState)
    ...
    .addEdge(["es_recall", "milvus_recall"], "merge") // 两边都跑完在进行合并处理
