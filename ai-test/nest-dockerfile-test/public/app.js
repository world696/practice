...
function render(review) {
  ...
  $('impact-list').innerHTML = review.impactAreas?.length ? review.impactAreas.map((x) => `<div class="impact-item"><div class="impact-main"><span class="impact-dot risk-dot-${esc(x.risk)}"></span><div><strong>${esc(x.name)}</strong><p>${esc(x.affected || x.reason)}</p><small>为什么：${esc(x.reason)}</small><small>证据：${x.files?.length || 0} 个文件 · ${(x.files || []).slice(0, 3).map(esc).join('、')}${x.files?.length > 3 ? ' 等' : ''}</small><small>建议：${(x.recommendedChecks || []).map(esc).join('、')}</small></div></div>${riskPill(x.risk)}</div>`).join('') : '<div class="no-impact">本次 diff 未命中已配置的影响规则</div>';
